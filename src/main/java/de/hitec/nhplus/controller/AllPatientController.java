...
public class AllPatientController {
    ...
    public void handleDelete() {
        ...
        if (selectedItem == null) {
            showAlert(Alert.AlertType.ERROR, "Keine Auswahl", "Kein Patient/in ausgewählt", "Bitte wählen Sie einen Patient/in aus!");
            return;
        }

        LocalDate creationDate = DateConverter.convertStringToLocalDate(selectedItem.getDateCreated());
        boolean confirmDelete = showConfirmationAlert("Patient/in Löschen", "Dieser Eintrag hat eine Aufbewahrungsfrist von 10 Jahren. Möchten Sie ihn wirklich löschen?");
        ...
        if (!DateUtils.isAtLeastTenYears(creationDate)) {
            showAlert(Alert.AlertType.ERROR, "Löschen Fehlgeschlagen", "Löschfehler", "Sie können nur Patient/innen löschen, die mindestens 10 Jahre alt sind.");
            return;
        }
